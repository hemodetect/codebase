...
public class ColorUtility {
    ...
    //using HSL or HSV space, building blendColors method
     ...
     * @return Lm
     */
    public static double calculateLightness(int R, int G, int B) {
        double r, g, b, Y, yr;
        // using illuminant D65 with normalization Y = 100
        double Yr = 100.0;
        // conversion from RGB to CIEXYZ values
        r = R / 255.0;
        g = G / 255.0;
        b = B / 255.0;
        if (r > 0.04045)
        ...
        // forward transformation from CIEXYZ to CIELAB values
        yr = Y / Yr;
        if (yr > 0.008856)
            yr = (float) Math.pow(yr, 1 / 3.);
        else
            yr = (float) ((7.787 * yr) + 16 / 116.0);
        //calculate cieLAB Lm (L measured) value
        double Lm = (116 * yr) - 16;
        return Lm;
    }
    /**
     ...
     * @return am
     */
    public static double calculateAm(int R, int G, int B) {
        double r, g, b, X, Y, xr, yr;
        // using illuminant D65 with normalization Y = 100
        double Xr = 95.047;
        double Yr = 100.0;
        // conversion from RGB to CIEXYZ values
        ...
        if (yr > 0.008856)
            ...
            yr = (float) ((7.787 * yr) + 16 / 116.0);
        //calculate cieLAB am (a measured) value
        double am = 500 * (xr - yr);
        return am;
    }
    /**
     ...
     * @return bm
     */
    public static double calculateBm(int R, int G, int B) {
        double r, g, b, Y, Z, yr, zr;
        // using illuminant D65 with normalization Y = 100
        double Yr = 100.0;
        double Zr = 108.883;
        // conversion from RGB to CIEXYZ values
        ...
        //calculate cieLAB bm (b measured) value
        double bm = 200 * (yr - zr);
        return bm;
    }
}